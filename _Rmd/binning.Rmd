...
# Does metagenomic binning work on metatranscriptomes?
  
Recently, I was curious whether metagenomic binning methods would work for metatranscriptome sequences. I didn't have any well-characterized metatranscriptomes laying around, so I decided to test this by using three publicly-available fungal transcriptomes that I found in the [Transcriptome Shotgun Assembly Database](https://www.ncbi.nlm.nih.gov/Traces/wgs/?term=tsa). I queried the database for transcriptomes that resemble those that I expect to isolate from olive oil, and found three good ones to work with:
...
```{r make-content, eval=TRUE, echo="TRUE", include="TRUE"}
...
```{r transcript-lengths, include = TRUE, echo = FALSE, eval = TRUE, message=FALSE, warnings = FALSE}
